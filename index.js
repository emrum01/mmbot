function mmbot() {
  ...
  const GOOGLE_DRIVE_PARENT_FOLDER_ID = data.GOOGLE_DRIVE_PARENT_FOLDER_ID
  const GOOGLE_DRIVE_CHILD_FOLDER_ID = data.GOOGLE_DRIVE_CHILD_FOLDER_ID
  ...
  function getMapsID() {
    ...
    const len = mapData.rsp.maps.total
    for (let i = 0; i < len; i++) {
      const mapCreated = mapData.rsp.maps.map[i].created
      const mapCreatedDate = dayjs.dayjs(mapCreated).format('YYYY-MM-DD')
      if (
        now === mapCreatedDate ||
        dayjs.dayjs(now).add(-1, 'd').format('YYYY-MM-DD') ===
          dayjs.dayjs(mapCreated).format('YYYY-MM-DD') ||
        dayjs.dayjs(now).add(-7, 'd').format('YYYY-MM-DD') ===
          dayjs.dayjs(mapCreated).format('YYYY-MM-DD') ||
        dayjs.dayjs(now).add(-30, 'd').format('YYYY-MM-DD') ===
          dayjs.dayjs(mapCreated).format('YYYY-MM-DD') ||
        dayjs.dayjs(now).add(-90, 'd').format('YYYY-MM-DD') ===
          dayjs.dayjs(mapCreated).format('YYYY-MM-DD')
      ) {
  ...
  function FindOrCreateFolder(folderName) {
    ...
    var folder = DriveApp.getFolderById(GOOGLE_DRIVE_PARENT_FOLDER_ID)
